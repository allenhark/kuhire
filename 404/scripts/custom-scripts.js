...
//function that handles dog changer
function dogChanger(){

	$("#dog-changer .toggle").on("click", function(){

		$(this).stop().animate({"left":"0px", "opacity":"0"},{duration:400, complete:function(){

			$("#dog-changer").stop().animate({"left":"0px"},{duration:400});

		}});

	});

	$("#dog-changer ul li a").on("click", function(){

		var rel = $(this).attr("rel");

		$(".dog").css("background-image","url(images/" + rel +")");

	});

	$("#dog-changer ul li a").on("mouseenter", function(){

		$(this).stop().animate({"opacity":"1"},{duration:400});

	});

	$("#dog-changer ul li a").on("mouseleave", function(){

		$(this).stop().animate({"opacity":"0.5"},{duration:400});

	});

}

//function that adjusts height at lower resolutions
function adjustHeight(){

	var wHeight = $(window).height();
	var defaultPlanetBotttom = -1100;
	var defaultDogBottom = 300;
	var difference;

	if(wHeight < 750){

		difference = 750 - wHeight;

		$(".planet").css("bottom", defaultPlanetBotttom - (difference));
		$(".dog-wrapper").css("bottom", defaultDogBottom - (difference));

	}
	else{

		$(".planet").css("bottom", defaultPlanetBotttom);
		$(".dog-wrapper").css("bottom", defaultDogBottom);
